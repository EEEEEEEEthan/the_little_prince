...
## 玩家触发器的交互方式（builtin 脚本不能 class_name，枚举放这里）。
enum InteractKind {
	NONE,
	PRESS,
	LONG_PRESS,
...
## A 键提示跟随点：相对 trigger 世界坐标向上（Y 负方向），提示本身不继承旋转。
const INTERACT_PROMPT_WORLD_Y: float = -38.0
...
const DARK_SOIL_BURROW_WIDTH: int = 12
const DARK_SOIL_BURROW_HEIGHT: int = 8
const BROWN_GLASS_BOTTLE_WIDTH: int = 10
const BROWN_GLASS_BOTTLE_HEIGHT: int = 18
const SLUMPED_WINE_DRINKER_WIDTH: int = 16
const SLUMPED_WINE_DRINKER_HEIGHT: int = 22
const DRUNKARD_BOTTLE_COUNT: int = 17
const HUNCHED_LEDGER_MERCHANT_WIDTH: int = 16
const HUNCHED_LEDGER_MERCHANT_HEIGHT: int = 22
const GOLD_STAR_GLASS_JAR_WIDTH: int = 12
const GOLD_STAR_GLASS_JAR_HEIGHT: int = 16
## 点灯人那颗：比故乡更小，两个人站不下。
const LAMPLIGHTER_PLANET_RADIUS: float = PLANET_RADIUS * 0.55
## 点灯人那颗星空自转：昼夜极快，仍跟作息走。
const LAMPLIGHTER_STAR_ROTATION_SPEED: float = 1.2
const BLACK_COAT_LAMPLIGHTER_WIDTH: int = 16
const BLACK_COAT_LAMPLIGHTER_HEIGHT: int = 22
const BLACK_POST_STREET_LAMP_WIDTH: int = 12
const BLACK_POST_STREET_LAMP_HEIGHT: int = 20
const BLACK_POST_STREET_LAMP_FRAME_COUNT: int = 2
const GRAY_BEARD_PARCHMENT_SCHOLAR_WIDTH: int = 16
const GRAY_BEARD_PARCHMENT_SCHOLAR_HEIGHT: int = 22
const STACKED_CREAM_INK_PAGES_WIDTH: int = 16
const STACKED_CREAM_INK_PAGES_HEIGHT: int = 14
const GEOGRAPHER_REPORT_STACK_COUNT: int = 3
