...
## 夜图(starfield)覆盖其上，按角度平滑混合实现昼夜更替。
...
## 时段关键角（星空相对角度，随自转递减）：0=正午、DAY_HALF_ARC=朝霞、π=午夜、
## TAU-DAY_HALF_ARC=夕阳、TAU 回到正午。正午/黎明段占 DAY_HALF_ARC，黄昏/夜晚被压缩。
...
## 各时段地平线色（贴图最黑处）：正午浅蓝白、朝霞淡蓝、午夜深黑、夕阳暮紫。
