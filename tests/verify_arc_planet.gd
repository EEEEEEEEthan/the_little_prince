...
func _check_b612_story(scene: Node, planet: Planet) -> int:
	...
	for line in opening:
		...
		if line.speaker == "小王子":
			opening_has_prince = true
		if line.text.contains("玻璃罩"):
			printerr("开场对白不应提前罩玻璃罩")
			failed += 1
			break
	...
	var overhead := story.overhead
	var overhead_body := overhead.get_node("Body") as Label
	var expected_first_pull_line := B612Lines.pull_shoot(B612Story.SHOOT_COUNT - 1)
	story.skip_cinematics = false
	...
	while (
		...
		if shoots[0].visible:
			...
			break
		await process_frame
	if not overhead.visible or overhead_body.text != expected_first_pull_line:
		printerr(
				"拔掉后应播放头顶叙事，实际 %s"
				% overhead_body.text
		)
